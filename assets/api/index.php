...
if($method === 'POST' && $resource === 'admin' && ($parts[1] ?? '') === 'items'){
    $body = json_decode(file_get_contents('php://input'), true) ?: [];
    $restaurant_id = isset($body['restaurant_id']) ? (int)$body['restaurant_id'] : 1;
    $name = trim($body['name'] ?? '');
    $slug = trim($body['slug'] ?? '') ?: strtolower(preg_replace('/[^a-z0-9]+/', '-', $name));
...
if($method === 'PUT' && $resource === 'admin' && ($parts[1] ?? '') === 'items' && isset($parts[2])){
    ...
    try{
        ...
        $sql = 'UPDATE menu_items SET name=?, slug=?, description=?, price=?, image=?, category_id=?, available=?, cook_time_minutes=? WHERE id=?';
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$name, $slug, $description, $price, $image, $category_id, $available, $prep_time, $itemId]);
...
if($method === 'PUT' && $resource === 'admin' && ($parts[1] ?? '') === 'categories' && isset($parts[2])){
    ...
    try{
        $sql = 'UPDATE categories SET name=?, slug=?, position=?, active=? WHERE id=?';
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$name, $slug, $position, $active, $categoryId]);
...
if($method === 'POST' && $resource === 'admin' && ($parts[1] ?? '') === 'tables'){
    $body = json_decode(file_get_contents('php://input'), true) ?: [];
    $restaurant_id = (int)($body['restaurant_id'] ?? 1);
...
if($method === 'PUT' && $resource === 'admin' && ($parts[1] ?? '') === 'tables' && isset($parts[2])){
    ...
    try{
        ...
        $check = $pdo->prepare('SELECT id FROM restaurant_tables WHERE restaurant_id = (SELECT restaurant_id FROM restaurant_tables WHERE id = ?) AND number = ? AND id != ?');
        $check->execute([$tableId, $number, $tableId]);
        if($check->fetch()) return json(['error'=>'table_number_exists'],400);
        
        $stmt = $pdo->prepare('UPDATE restaurant_tables SET number=?, description=?, active=? WHERE id=?');
        $stmt->execute([$number, $description, $active, $tableId]);
...
if($method === 'GET' && $resource === 'admin' && ($parts[1] ?? '') === 'orders' && isset($parts[2])){
    $orderId = (int)$parts[2];
    $stmt = $pdo->prepare('SELECT o.id, o.restaurant_id, o.table_id, o.session_token, o.status, o.total, o.notes, o.created_at, o.updated_at, rt.number as table_number FROM orders o LEFT JOIN restaurant_tables rt ON rt.id = o.table_id WHERE o.id = ? LIMIT 1');
...
if($method === 'DELETE' && $resource === 'admin' && ($parts[1] ?? '') === 'ratings' && isset($parts[2])){
    ...
    try{
        // get item_id first
        $ratStmt = $pdo->prepare('SELECT item_id FROM ratings WHERE id = ? LIMIT 1');
        $ratStmt->execute([$ratingId]);
        $rat = $ratStmt->fetch();
