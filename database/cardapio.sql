...
CREATE TABLE `admin_users` (
  ...
  `locked_until` timestamp NULL DEFAULT NULL COMMENT 'Conta bloqueada até esta data/hora após demasiadas tentativas falhadas',
...
--
-- (sem dados de exemplo — a tabela começa vazia)
...
CREATE TABLE `visits` (
  ...
  `last_seen_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Actualizado a cada "heartbeat" do cliente; usado para expirar a sessão da mesa após inactividade',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
ALTER TABLE `notifications`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_notifications_restaurant` (`restaurant_id`),
  ADD KEY `idx_notifications_user` (`user_id`),
  ADD KEY `idx_notifications_lida` (`lida`);
...
--
ALTER TABLE `notifications`
  ADD CONSTRAINT `fk_notifications_restaurant` FOREIGN KEY (`restaurant_id`) REFERENCES `restaurants` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_notifications_user` FOREIGN KEY (`user_id`) REFERENCES `admin_users` (`id`) ON DELETE CASCADE;
